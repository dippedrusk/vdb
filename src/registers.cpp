...
namespace {
	template <class T> vdb::byte128 widen(const vdb::register_info& info, T t) {
		using namespace vdb;
...
vdb::registers::value vdb::registers::read(const register_info& info) const {
	...
	if (info.format == register_format::uint) {
		switch (info.size) {
			...
			default: vdb::error::send("Unexpected register size");
		}
	}
...
void vdb::registers::write(const register_info& info, value val) {
	auto bytes = as_bytes(data_);
	...
	std::visit([&](auto& v) {
		if (sizeof(v) <= info.size) {
			...
			std::cerr << "vdb::register::write called with "
	...
	if (info.type == register_type::fpr) {
		...
		// we do this to align the high 8-bit registers, i.e., set the lowest 3 bits to 0
